...
def cleanup_text(text: str) -> str:
    clean: str = remove_self_mentions(text)
    clean = clean[: Config.MAX_MESSAGE_LENGTH]
    clean = clean.strip()
    return clean
...
class Pipeline:
    ...
    def reply(self, message: Message) -> str | None:
        ...
        text: str = cleanup_text(message.content)
        tokenized_message: Tensor = self.tokenizer.encode(text)
